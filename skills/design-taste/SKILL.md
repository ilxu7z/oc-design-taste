...
name: design-taste
description: >-
  设计品味校准框架 v2。三旋钮推理 + Context-Aware 痕迹目录 + 机械交付检查 + 重设计协议。
  适配B端制造业/画册/官网/生图/排版等视觉类任务。
  触发：设计/UI/前端/页面/画册/图片/官网/生图/样式/排版/布局/重新设计。
  内建容错：交付前二进制检查 → 不合格自动打回。
---

# Design Taste v2 — 设计品味校准框架
...
1. **载体类型** — 产品画册 / 官网首页 / 产品页 / 场景页 / 生图 / Logo / 海报 / 邮件模板
...
6. **约束条件** — 中英双语 / 多语言 / 三端响应式 / 移动优先 / 打印尺寸 / 字数限制
...
**字体配对知识**（选了标题字体，正文用同一字族的正文字重，或这些配对）：
- Geist + Geist Mono
...
| 长列表换组件 | >5 项不用 `<ul>` + `divide-y`，用卡片网格/Tab/手风琴 | 数据表 |
...
> **核心区别**：v1 是二进制禁令表。v2 每条都标注了**为什么禁**和**什么时候可以突破**。
...
| 4 | 动效层 | 中 | 中——加旋钮对应的微交互 |
| 5 | Hero & 关键 section 重组 | 高 | 高——用 Section 5 词汇表 |
